...
LLM $\equiv$ H1 $\equiv$ H2 $\equiv$ H3 & BLEU-4, METEOR, chrF++, ROUGE-L & $p > 0{,}05$ \\
H1 $\equiv$ H2 $\equiv$ H3 & BLEU-4, chrF++, ROUGE-L & $p > 0{,}01$ \\
\hline
\end{tabular}
...
El comportamiento de BERTScore merece una observación metodológica. El test de Friedman detecta diferencias significativas ($\chi^2 = 78{,}05$, $p < 0{,}0001$), pero ninguna comparación por pares alcanza significancia tras la corrección de Holm-Bonferroni ($p \geq 0{,}08$). Esta aparente discrepancia se explica por la distribución de los datos: todos los motores obtienen valores de BERTScore próximos a 1,0 (entre 0,987 y 1,000), de modo que Friedman detecta un orden de ranking consistente ---la gramática siempre primera, los híbridos siempre por debajo--- pero las diferencias absolutas son tan pequeñas que los tests por pares no las consideran significativas. Este resultado confirma que, para el dominio acotado de CAA con frases cortas, BERTScore alcanza un techo donde pierde capacidad discriminativa: todas las salidas son semánticamente equivalentes a la referencia, independientemente de las diferencias superficiales que otras métricas sí capturan.
