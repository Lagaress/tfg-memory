...
Hybrid2 replica la estructura de Hybrid1 pero sustituye la gramática pura por el motor de embeddings, que combina gramática con embeddings semánticos, como fuente de la frase inicial (Figura~\ref{fig:hibridacion2}). La frase generada por este motor, que ya incorpora refinamiento de preposiciones y ranking semántico, se envía al mismo post-editor LLM con las mismas ocho reglas.

La única diferencia con Hybrid1 es el motor que produce la frase intermedia: gramática pura en Hybrid1, gramática enriquecida con embeddings en Hybrid2. El post-editor es idéntico en ambos casos (mismo prompt, mismo modelo), lo que convierte la comparación en un experimento controlado: cualquier diferencia en los resultados se atribuye directamente a la contribución de los embeddings.
...
\begin{figure}[ht]
    ...
    \caption{Pipeline de Hybrid2: el motor de gramática enriquecida con embeddings produce la frase inicial y el mismo post-editor LLM la refina con ocho reglas de fidelidad; el \textit{fallback} es idéntico al de Hybrid1.}
...
Hybrid3 se basa en el paradigma \textit{Generate-Filter-Rank}, formalizado por Sharma et al.~\cite{sharma2019overgenerate}: se generan múltiples candidatas, se filtran por criterios de calidad y se selecciona la mejor según una función de ranking. A diferencia de Hybrid1 y Hybrid2, que mejoran la salida de un motor individual, Hybrid3 busca la mejor frase entre todas las que los motores pueden producir. El pipeline se ejecuta en tres etapas secuenciales (Figura~\ref{fig:hibridacion3}).

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/hibridacion3.png}
    \caption{Pipeline de Hybrid3 (\textit{Overgenerate-and-Rank}): cinco generadores (gramática formal, embeddings, LLM, Hybrid1 y Hybrid2) producen una candidata cada uno; el filtro de fidelidad selecciona las que cubren el 100\,\% de los pictogramas de entrada y el ranking por perplejidad (GPT-2 en español) elige la más natural.}
    \label{fig:hibridacion3}
\end{figure}

La primera etapa genera una candidata con cada uno de los cinco motores disponibles: gramática formal, embeddings, LLM, Hybrid1 y Hybrid2. Si algún motor falla, se descarta su candidata sin afectar al resto.
...
Hybrid3 acumula la latencia de todos los motores que invoca, lo que la convierte en la opción más lenta. El paradigma de sobregeneración y ranking solo aporta valor cuando las candidatas son diversas y la función de ranking tiene granularidad suficiente para discriminar entre ellas; en un dominio con frases cortas y vocabulario restringido, las candidatas tienden a converger y el ranking pierde capacidad discriminativa. El umbral de fidelidad de cobertura completa puede descartar frases naturales que omiten un pictograma con representación implícita: ``Quiero agua'' para la entrada ``yo, quiero, agua'' podría fallar si el sistema no reconoce el sujeto implícito.
