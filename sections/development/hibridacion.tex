...
\subsection{Hibridación 1: Gramática + LLM como post-editor}
\label{sec:h-hybrid1}
...
\begin{figure}[ht]
    ...
    \caption{Pipeline de la Hibridación 1: la gramática formal produce la frase inicial y el LLM la pos-edita con ocho reglas de fidelidad; si la gramática no reconoce la entrada, el LLM la genera desde cero como \textit{fallback}.}
...
\subsection{Hibridación 2: (Gramática + Embeddings) + LLM como post-editor}
\label{sec:h-hybrid2}
...
\begin{figure}[ht]
    ...
    \caption{Pipeline de la Hibridación 2: el motor de gramática enriquecida con embeddings produce la frase inicial y el mismo post-editor LLM la refina con ocho reglas de fidelidad; el \textit{fallback} es idéntico al de H1.}
    \label{fig:hibridacion2}
\end{figure}

\subsection{Hibridación 3: Overgenerate-and-Rank multimotor}
\label{sec:h-hybrid3}
...
\begin{figure}[ht]
    ...
    \caption{Pipeline de la Hibridación 3 (\textit{Overgenerate-and-Rank}): cinco generadores (gramática formal, embeddings, LLM, H1 y H2) producen una candidata cada uno; el filtro de fidelidad selecciona las que cubren el 100\,\% de los pictogramas de entrada y el ranking por perplejidad (GPT-2 en español) elige la más natural.}
