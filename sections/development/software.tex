...
La separación entre generación y post-procesamiento sigue el patrón \textit{plan-then-realize}~\cite{moryossef2019stepbystep}, que se aplica en Hybrid1 y Hybrid2: la gramática o los \textit{embeddings} producen una frase estructurada y el LLM la mejora.

Hybrid3 implementa el patrón \textit{overgenerate-and-rank}~\cite{sharma2019overgenerate}, que desacopla la generación de la selección y permite añadir o retirar generadores sin modificar la lógica de \textit{ranking}.
...
\begin{itemize}
    ...
    \item API de ElevenLabs (modelo \textit{Flash v2.5}) para la síntesis de voz neuronal de la aplicación, con \texttt{expo-speech} como respaldo \textit{offline}.
